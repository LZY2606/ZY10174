...
func TestParseDupKeyDefaultKeepsLast(t *testing.T) {
	...
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 2}, v, "default policy keeps the last value")
	dups, total := p.DupKeyDiags()
	tt.Equal(t, 0, total, "default policy records no diagnostics")
	tt.Equal(t, 0, len(dups))
}
...
func TestParseDupKeyFirst(t *testing.T) {
	...
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 1, "b": map[string]any{"c": 1}}, v,
...
func TestParseDupKeyFirstNestedValue(t *testing.T) {
	...
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 1, "c": []any{3}}, v,
...
func TestParseDupKeyFirstNestedDupInDupValue(t *testing.T) {
	...
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 1}, v,
...
func TestParseDupKeyReport(t *testing.T) {
	...
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 3}, v, "report keeps the last value")
	dups, total := p.DupKeyDiags()
...
func TestParseDupKeyEscapedKeys(t *testing.T) {
	...
	v, err := p.Parse([]byte(src))
	tt.Nil(t, err)
	tt.Equal(t, map[string]any{"a": 2}, v)
	dups, total := p.DupKeyDiags()
...
func TestParseDupKeySeparateDocuments(t *testing.T) {
	var p oj.Parser
	p.DupKey.Policy = ojg.DupKeyReport
	var got []any
	_, err := p.Parse([]byte(`{"a":1}{"a":2}`), func(v any) { got = append(got, v) })
	tt.Nil(t, err)
	tt.Equal(t, 2, len(got))
	_, total := p.DupKeyDiags()
	tt.Equal(t, 0, total, "keys in separate documents are not duplicates")
}
...
func TestParseDupKeyReaderChunked(t *testing.T) {
	...
	for _, size := range []int{1, 2, 5, 4096} {
		...
		tt.Equal(t, map[string]any{"a": 3, "b": map[string]any{"c": 2}}, v, "chunk size", size)
		dups, total := p.DupKeyDiags()
...
func TestParseDupKeyReaderSplitMidEscape(t *testing.T) {
	...
	for size := 1; size <= 8; size++ {
		...
		v, err := p.ParseReader(&chunkReader{content: []byte(src), n: size})
		tt.Nil(t, err, "chunk size", size)
		tt.Equal(t, map[string]any{"a": 2}, v, "chunk size", size)
		dups, total := p.DupKeyDiags()
...
func TestParseDupKeyOnDupCallback(t *testing.T) {
	var seen []ojg.DupKeyDiag
	var p oj.Parser
	p.DupKey = ojg.DupKeyOptions{
		Policy:   ojg.DupKeyReport,
		MaxDiags: 1,
		OnDup:    func(d ojg.DupKeyDiag) { seen = append(seen, d) },
	}
	_, err := p.Parse([]byte(`{"a":1,"a":2,"a":3}`))
	tt.Nil(t, err)
	tt.Equal(t, 2, len(seen), "callback sees duplicates beyond the retained limit")
	dups, _ := p.DupKeyDiags()
